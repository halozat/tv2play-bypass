...
export default class TV2PlayParser {
    ...
    getAndParseManifest(uuid, url) {
        return new Promise((resolve, reject) => {
            this.client
                ...
                const stream = parser?.manifest?.playlists
                    .slice(-1)
                    .map(({ attributes, uri }) => {
    ...
    getStreamingURL(playerId) {
        return new Promise((resolve, reject) => {
            this.client
                ...
                // yeah, it expires...
                const expiry = new Date(parseInt(new URL(streamingURL)?.searchParams?.get?.("e")) * 1000);
                resolve({
                    expiry,
                    streamingURL,
                });
    ...
    getSeriesAndPartData(series, part) {
        return new Promise((resolve, reject) => {
            this.client
                ...
                let part = this.formatSeriesPart(data?.seriesInfo?.seasonNr, data?.seriesInfo?.episodeNr);
    ...
    searchByUrl(url) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                const { playerId, title, part } = await this.getSeriesAndPartData(series, partRaw);
